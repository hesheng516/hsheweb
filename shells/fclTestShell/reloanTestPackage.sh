...
DIST_USER="reloan"
# 目标目录的根目录
DIST_DIR="/home/reloan"
DIST_IP1="172.16.68.78"
DIST_IP2="172.16.68.79"
DIST_DIR1=${DIST_USER}@${DIST_IP1}:${DIST_DIR}
DIST_DIR2=${DIST_USER}@${DIST_IP2}:${DIST_DIR}
...
function func_service_build()
{
	...
	# -q 为静默编译   -Pbank 指定使用的MAVEN配置文件（如果要使用-Ptest 是否冲突？）。
	mvn clean install -Dmaven.test.skip=$SKIPTEST  -q -Pbank
...
# 拷贝停止脚本和启动脚本到服务器
scp ${BASE_DIR}/shells/fclTestShell/*.sh  ${DIST_DIR1}/shbin/
scp ${BASE_DIR}/shells/fclTestShell/*.sh  ${DIST_DIR2}/shbin/
ssh ${DIST_USER}@${DIST_IP1} "chmod 755 ${DIST_DIR}/shbin/*.sh "
ssh ${DIST_USER}@${DIST_IP2} "chmod 755 ${DIST_DIR}/shbin/*.sh "

# 停止服务（ TODO:如果仅启动某几个服务，就不全部停止）
# 暂时使用kill 命令进行全部停止。
#ssh ${DIST_USER}@${DIST_IP1} "${DIST_DIR}/shbin/stopReloanService.sh  -p ${SERVICE_TO_STOPS} " 
#ssh ${DIST_USER}@${DIST_IP1} "${DIST_DIR}/shbin/stopReloanService.sh   " 
#ssh ${DIST_USER}@${DIST_IP2} "${DIST_DIR}/shbin/stopReloanService.sh   " 
STOP_PARAMETERS=""
realParameters=()
i=0
...
if [ ${realParameters}"x" == "x" -o ${realParameters}"x" == "x" ] ; then
	#sh ${DIST_USER}@${DIST_IP} "${BASE_DIST_DIR}/stopReloanService.sh   " 
    ssh ${DIST_USER}@${DIST_IP1} "${DIST_DIR}/shbin/stopReloanService.sh   " 
	ssh ${DIST_USER}@${DIST_IP2} "${DIST_DIR}/shbin/stopReloanService.sh   " 
else
#如果单独构建项目参数不是空,那么就构建指定项目,并只停止指定项目
	...
	do
    	ssh ${DIST_USER}@${DIST_IP1} "${DIST_DIR}/shbin/stopReloanService.sh  -p ${stop_parameter} " 
    	ssh ${DIST_USER}@${DIST_IP2} "${DIST_DIR}/shbin/stopReloanService.sh  -p ${stop_parameter} " 
    done
fi
...
do	
        ...
        scp target/${service}-0.0.1-SNAPSHOT.jar ${DIST_DIR1}/app/reloan/
        
        # 拷贝到目标 目录 （）
        scp target/${service}-0.0.1-SNAPSHOT.jar ${DIST_DIR2}/app/reloan/
...
ssh ${DIST_USER}@${DIST_IP1} "${DIST_DIR}/shbin/startReloanService.sh " 
ssh ${DIST_USER}@${DIST_IP2} "${DIST_DIR}/shbin/startReloanService.sh " 
#cp ${BASE_DIR}/*/*/*/*SNAPSHOT.jar ${DIST_DIR}/

echo "打包完成！请检查目标目录。${DIST_DIR1}/app/reloan/   ${DIST_DIR2}/app/reloan/"
date +%F" "%H:%M:%S
