...
function show_usage()
{
	...
	echo "-p是停止某几个服务,例如-p \"sysman\"\\ \"wxpreweb\"\\ \"gateway\""
	#-p是停止某几个服务,例如-p "sysman"\ "wxpreweb"\ "gateway"
	echo "		指定参数: \n		"
...
if [ ${BUILD_PROJECTS}"X" = "X" ]  ; then
	...
	#判断所有项目是否停止
	STARTJAVA=$(ps -aux | grep "SNAPSHOT" | awk '{ if($11=="java") { print $2} } ' )
	#如果有项目在运行杀掉项目
	if [ ! -z "${STARTJAVA}" ] ; then
		echo "当前还在运行的程序有"`ps -aux|grep SNAPSHOT|grep -v grep|wc -l`"个"
		echo `ps -aux|grep SNAPSHOT|grep -v grep`
	...
	do	
			# 循环检查单个服务是否停止
			ps -aux|grep ${service}-0.0.1-SNAPSHOT.jar|awk ' { if ($11=="java") { print $2 } }' |xargs kill -9
	done
